...
Notation: Follows ISO-5807 conventions for user-facing flows — decisions are binary (Yes/No) and explicit. Mermaid cannot render every ISO 5807 shape, so input, output, and document nodes are labeled textually. See: https://cdn.standards.iteh.ai/samples/11955/1b7dd254a2a54fd7a89d616dc0570e18/ISO-5807-1985.pdf

Note: Engagium is professor-only. There is no student or participant login surface. The program flowchart is intentionally split by area of concern so each user journey remains readable.
...
---
flowchart TB
		Start(["Start"]) --> Auth["Auth and recovery flow"]
		Auth --> Hub["Dashboard hub and navigation"]
		Hub --> Classes["Class and roster management"]
		Hub --> Sessions["Session monitoring and history"]
		Hub --> Analytics["Analytics and review"]
		Hub --> Settings["Settings and extension tokens"]
		Hub --> Zoom["Zoom bridge and OAuth flow"]
		Hub --> Extension["Chrome extension meeting tracking"]
		Classes --> End(["End"])
		Sessions --> End
		Analytics --> End
		Settings --> End
		...
		class Auth,Hub,Classes,Sessions,Analytics,Settings,Zoom,Extension io;
		class Start,End terminal;
```

Detailed concern-specific diagrams:
